...
fun calcCommission(amount: Int, type: String = "VK Pay", prevTransfersInMonth: Int = 0, prevTransfersInDay: Int = 0): Int {
    when (type) {
        "Mastercard", "Maestro" -> {
            if ((prevTransfersInMonth+amount) > LIMIT_IN_MONTH_WITHOUT_COMMISSION)
                return (amount * COMMISSION_PERCENT_MASTERCARD_MAESTRO / 100 + ADDITIONAL_COMMISSION_MASTERCARD_MAESTRO).toInt()
        }
        "Visa", "Мир" -> {
